...
class SliderPropertiesMM: UIViewController, MSCircularSliderDelegate {
    ...
    @IBOutlet weak var sliderMM: MSCircularSlider!
    @IBOutlet weak var valueMMLbl: UILabel!
    ...
    override func viewDidLoad() {
        ...
        sliderMM.delegate = self
        
        currentValue = sliderMM.currentValue
        valueMMLbl.text = String(format: "%.1f", currentValue)
    ...
    @objc func updateSliderValue() {
        sliderMM.currentValue += animationReversed ? -1.0 : 1.0
        
        if sliderMM.currentValue >= sliderMM.maximumValue {
            animationTimer?.invalidate()
            // Reverse animation
            animationReversed = true
            animationTimer = Timer.scheduledTimer(timeInterval: 0.01, target: self, selector: #selector(updateSliderValue), userInfo: nil, repeats: true)
        }
        else if sliderMM.currentValue <= sliderMM.minimumValue && animationReversed {
            // Animation ended
            animationTimer?.invalidate()
        }
    }
    ...
    @objc func updateTime() {
        valueMMLbl.text = "\(timeFormatted(Int(totalTime)))"
    ...
    @IBAction func startTimerPressed(_ sender: UIButton) {
        ...
        totalTime = sliderMM.currentValue * 60
        startTimer()
    }
